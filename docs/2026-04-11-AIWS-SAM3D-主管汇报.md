...
- `scripts/sam3d_aiws52_batch.py`：SAM3D 全量批处理脚本，支持 resume、shard、多卡与运行指标记录
- `scripts/sam3d_run_metrics_analysis.py`：统计并分析 SAM3D 运行结果
- `scripts/sam3d_to_cadrille_e2e.py`：将 SAM3D 网格输出整理为 Cadrille 各模式所需的 mesh-derived 输入，下游汇总也由该脚本产出
