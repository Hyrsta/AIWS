...
- 数据集：`aiws5.2-usable-materialized`
- 正式任务总数：**1418**
- 子集分布：
  - `V1`: 593
  - `V2`: 524
...
因此，后续把数据逐步整理成了 3 层视图：

#### （1）`aiws5.2-usable-split/`

这是**保留 train / val 划分**的非破坏性符号链接视图，结构为：

- `train/`、`val/`
- 每个 split 下再分 `V1/`、`V2/`、`NEW/`
- 每个 subset 下再分 5 类工件：
  - `cover_plate`（盖板）
  - `square_tube`（方管）
  - `h_beam`（H 型钢）
...
这是**不强调 train / val，只强调“可用于正式重建”**的清洗视图，结构为：

- 顶层按 `V1 / V2 / NEW`
- 子层按工件类别分组
- 把不适合直接进入主干实验的样本移到 `misc/`

其中：

- `misc/multi_instance/`：一张图中有多个实例
- `misc/multi_label/`：一张图中有多个不同类别
- `misc/unannotated_images/`：有图像但无标注
...
这是**最终正式实验使用的数据集根目录**，也是 SAM3D 批处理脚本实际读取的目录。

它延续了 `subset / workpiece` 的清晰组织方式，并补充了更适合正式运行的配套文件，例如：
...
以 `aiws5.2-usable-materialized/` 为例：
...
- `masks/`：由标注 polygon 栅格化生成的实例 mask
...
**原始平铺资源池 → split-aware 视图 → cleaned usable 视图 → 正式 materialized 实验视图。**
