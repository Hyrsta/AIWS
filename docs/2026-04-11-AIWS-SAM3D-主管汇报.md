...
当前主实验主干采用的是更干净的单实例样本，因此 `misc/` 中的样本单独列出。
...
### 2.3 项目结构（AIWS 在线/离线划分与离线 CAD 管线）

**AIWS 分为 online pipeline 和 offline pipeline 两部分**，而本文讨论的 CAD 重建管线就是其中的 **offline pipeline**。
...
- SAM3D：`/ssd1/rxl/zhankaiming/AIWS/repos/sam-3d-objects`
    - 当前直接依赖的官方/默认入口主要包括：`demo.py`、`notebook/inference.py`、`checkpoints/hf/pipeline.yaml`
- Cadrille：`/ssd1/rxl/zhankaiming/AIWS/repos/cadrille`
    - 当前直接依赖的官方/默认脚本主要包括：`test.py`、`evaluate.py`、`convert_cadquery.py`
...
- `scripts/build_aiws52_usable_view.py`：构建 `aiws5.2-usable/` 的正式数据视图
- `scripts/generate_aiws52_instance_masks.py`：从标注生成实例级 mask/中间数据
- `scripts/run_sam3d_aiws52_batch.py`：SAM3D 全量批处理脚本，支持 resume、shard、多卡与运行指标记录
- `scripts/analyze_sam3d_run_metrics.py`：统计并分析 SAM3D 运行结果
- `scripts/run_sam3d_to_cadrille_e2e.py`：将 SAM3D 网格输出整理为 Cadrille 各模式所需的 mesh-derived 输入，下游汇总也由该脚本产出
...
> 《环境搭建与推理 SOP》中已补充 `flash_attn` 的源码编译命令、校验方法，以及 A6000 上必须显式设置后端环境变量的说明。
...
口径说明：本节中 Cadrille 的运行时间、折算吞吐和端到端时间统一按**最终成功输出**统计。`送入 Cadrille 的样本数` 只作为完整性说明，不再单独给出按 attempted sample 计的效率口径。
...
- 模态：`pc`
- 输入：SAM3D 导出的 STL
- `n_samples=5`（每个输入生成 5 个候选，再由 `evaluate` 选最优，因此墙钟时间更长）
- `batch_size=64`
- 4 卡并行
- 选择策略：`evaluate`
...
| 指标 | 数值 |
|---|---:|
| 送入 Cadrille 的样本数（仅作完整性说明） | 1418 |
| 最终成功输出样本数 | 1418 |
| 成功率 | 100.0% |
| 墙钟时间 | 约 80.1 分钟 |
| 折算吞吐（按成功输出） | 约 1061.7 成功输出/小时 |

补充质量指标（按 shard summary 均值汇总，并按 Cadrille 论文口径展示）：

| 指标 | 数值 |
|---|---:|
| 平均 IoU (%) | 2.12 |
| 中位 Chamfer 距离 (×10³) | 38.00 |
...
- 不是单卡串行逐样本时延
- 对 SAM3D 而言，这里仍等于 `1418` 个正式样本；对 Cadrille 则统一按最终成功输出数统计

先看各阶段的墙钟折算：

- SAM3D：约 `1.60` 小时，折算约 **4.07 s / 样本**
- Cadrille-PC：约 `80.1` 分钟，折算约 **3.39 s / 成功样本**
- Cadrille-IMG：约 `25.9` 分钟，折算约 **1.28 s / 成功样本**

据此可得当前全链路的平均端到端时间：

| 链路 | 计算方式 | 折算平均端到端时间 |
|---|---:|---:|
| SAM3D → Cadrille-PC | `4.07 + 3.39` | **7.46 s / 成功样本** |
| SAM3D → Cadrille-IMG（按成功输出样本数） | `(SAM3D总墙钟 + IMG总墙钟) / 1213` | **6.04 s / 成功样本** |

解释如下：

1. **按当前“成功输出”口径**，IMG 链路的端到端速度仍然更快。
2. **IMG 的速度优势没有 raw attempted-sample 视角下那么大**，因为当前成功率约 `85.54%`。
3. **PC 链路当前更慢，一个直接原因是 `n_samples=5`**，即每个输入都会生成 5 个候选再做筛选，因此计算量更高；但它当前成功率是 `100%`，更适合作为稳定性优先的正式基线。
